...
import { getRideHistoryById, updateRideHistoryById } from 'apiSdk/ride-histories';
...
function RideHistoryEditPage() {
  ...
  const handleSubmit = async (values: RideHistoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateRideHistoryById(id, values);
      mutate(updated);
      resetForm();
